	...
	UPDATE language_de_DE SET Text = 'The Great Scientist can construct the special Academy improvement which, when worked, produces lots of [ICON_RESEARCH] Science and increases the value of the Scientist''s Discover ability by 10%. Further, a Great Scientist can give you a considerable boost towards your next tech. The Great Scientist is expended when used in any of these ways.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_ENGINEER_STRATEGY' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_UNITS' AND Value= 1 );
	...
	UPDATE language_de_DE SET Text = 'The Great Scientist can construct the special Academy improvement which, when worked, produces lots of [ICON_RESEARCH] Science and increases the value of the Scientist''s Discover ability by 10%. Further, a Great Scientist can give you a considerable boost towards your next tech. Finally, the Great Scientist can start a Golden Age. The Great Scientist is expended when used in any of these ways.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_SCIENTIST_STRATEGY' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_UNITS' AND Value= 1 );
	...
	UPDATE language_de_DE SET Text = 'Der Große Admiral kann die Ozeane sofort kostenlos überqueren, was ihn zu einem nützlichen (wenn auch verwundbaren) Forschungsschiff macht. Darüber hinaus kann der Große Admiral, während er sich im eigenen Territorium befindet, auf eine "Entdeckungsreise" geschickt werden. Dies verbraucht die Einheit und stellt Ihnen zwei Kopien eines Luxusguts zur Verfügung, das auf der aktuellen Karte nicht verfügbar ist.[NEWLINE][NEWLINE]Der Große Admiral verfügt über zwei Ladungen der Fähigkeit, jede Marineeinheiten und Einheiten zu Wasser im gleichen Feld und in angrenzenden Feldern sofort zu reparieren. Der Große Admiral wird verbraucht, wenn er die letzte Ladung für seine Aktion benutzt. Der Große Admiral bringt auch 15% Kampfbonus für alle Marineeinheiten im Spielerbesitz innerhalb von 2 Geländefeldern. Der Große Admiral wird NICHT verbraucht, wenn er den Bonus gewährt. Wenn der Große Admiral für eine Entdeckungsreise oder Reparatur aufgewendet wird, erhöht sich die Versorgungsgrenze für militärische Einheiten um 2.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_ADMIRAL_STRATEGY' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_UNITS' AND Value= 1 );

	UPDATE language_de_DE SET Text = 'Dieser Befehl verbraucht eine von zwei Ladungen des Großen Admirals und heilt all Eure Marineeinheiten sowie die Einheiten zu Wasser auf diesem und allen angrenzenden Geländefeldern. Wenn dies die letzte Ladung ist, erhöht sie auch Eure Versorgungsgrenze für militärische Einheiten um 2.' WHERE Tag = 'TXT_KEY_MISSION_REPAIR_FLEET_HELP' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_UNITS' AND Value= 1 );

	-- General
	UPDATE language_de_DE SET Text = 'Der Große General kann die Spezialmodernisierung Zitadelle bauen, die jeder Einheit, die sich in ihr befindet, einen immensen Verteidigungsbonus verleiht, feindlichen Einheiten auf angrenzenden Geländerfeldern Schaden zufügt, alle angrenzenden Geländefelder annektiert sowie Eure Versorgungsgrenze für militärische Einheiten um 2 erhöht. Der Große General wird verbraucht, wenn er die Zitadelle baut. Der Große General gewährt allen Landeinheiten des Spielers im Umkreis von 2 Geländefeldern einen Kampfbonus von 15%. Der General wird NICHT verbraucht, wenn er den Bonus gewährt.' WHERE Tag = 'TXT_KEY_UNIT_GREAT_GENERAL_STRATEGY' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_UNITS' AND Value= 1 );

	-- Archaeologist Text

	UPDATE Language_de_DE SET Text = 'Maximum [COLOR_POSITIVE_TEXT]three[ENDCOLOR] active Archaeologists per player at any one time. Archaeologists are a special subtype of Worker that are used to excavate Antiquity Sites to either create Landmark improvements or to extract Artifacts to fill in Great Work of Art slots in Museums, Palaces, Hermitages, and selected Wonders. Archaeologists may work in territory owned by any player. They are consumed once they complete an Archaeological Dig at an Antiquity Site. Archaeologists may not be purchased with [ICON_GOLD] Gold and may only be built in a city with a Public School.' WHERE Tag = 'TXT_KEY_UNIT_HELP_ARCHAEOLOGIST' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_UNITS' AND Value= 1 );
	...
	UPDATE Language_de_DE
	SET Text = 'Ihr könnt einen Großen Händler einsetzen, um eine Kleinstadt zu bauen. Wenn bewirtschaftet, produziert die Kleinstadt eine Menge Gold und Nahrung pro Runde für seine Stadt und verlängert außerdem die "Feiertage" durch Große Händler Handelsmissionen.[NEWLINE][NEWLINE]Kleinstädte erhalten zusätzliches [ICON_GOLD] Gold und [ICON_PRODUCTION] Produktion, wenn sie auf einer Straße oder Eisenbahnstrecke errichtet wurden, die zwei eigene Städte miteinander verbindet.[NEWLINE][NEWLINE]Erhaltet zusätzliches [ICON_GOLD] Gold und [ICON_PRODUCTION] Produktion (+1 vorindustrielles Zeitalter, +2 Industriezeitalter und später), wenn ein Handelsweg (national oder international) über diese Kleinstadt führt.'
	...
	UPDATE Language_de_DE
	SET Text = 'Fernkampfeinheit des Atomzeitalters, die hohen Schaden bei gepanzerten Einheiten verusacht'
